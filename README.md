...
Access historical data and state changes for any Home Assistant entity. This tool allows you to query past values, status changes, and trends for sensors, lights, switches, and other entities.

#### Requirements

* Home Assistant's built-in history component must be enabled (default in most installations).
* The entity must have history recording enabled.

#### Configuration Steps

1. Select "Entity History" during setup.
2. No additional configuration required.

#### Usage Examples

The LLM can use this tool to answer questions like:
* "What was the temperature in the living room last night?"
...
* **Flexible time ranges**: Specify exact start/end times or use defaults (last 12 hours)
* **Smart summarization**: Automatically summarizes large datasets with statistics
* **State-based queries**: Get history since a specific entity state change
* **All entity types**: Works with sensors, binary sensors, lights, switches, and more
* **LLM-friendly output**: Formatted for easy consumption by language models

#### Options

| Setting | Required | Description |
|---------|----------|-------------|
| `Entity History` | ✅ | Enable the Entity History tool (no additional configuration) |
